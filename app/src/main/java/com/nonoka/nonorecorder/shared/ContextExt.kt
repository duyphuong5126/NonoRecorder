...
fun Context.createSharedAudioFile(mp3File: File) {
    if (SDK_INT < Q) {
        try {
            val directory = Environment.getExternalStoragePublicDirectory(DIRECTORY_MUSIC)
            val file = File(directory, "$externalRecordedFolder/${mp3File.name}").apply {
                createNewFile()
            }
            file.outputStream().let { outputStream ->
                writeFileToOutputStream(mp3File, outputStream)
                Timber.d("Recording>>> Saved file ${mp3File.name} into ${file.path}")
            }
        } catch (error: Throwable) {
            Timber.d("Recording>>> Could not save file ${mp3File.name} into music folder with error $error")
            throw error
        }
    } else {
